...
// ========================================
// MISSING UTILITY FUNCTIONS - HELPERS
// ========================================
...
function parseCurrencyToNumber(currencyStr) {
    ...
    const cleaned = currencyStr.replace(/[₹,\s]/g, '').trim();
    const num = parseFloat(cleaned);
    return isNaN(num) ? 0 : num;
}

// Set processing state on a cart item
function setItemProcessing(dishId, isProcessing) {
    const itemEl = document.querySelector(`.cart-item[data-dish-id="${dishId}"]`);
    if (!itemEl) return;
    
    const spinner = itemEl.querySelector('.quantity-spinner');
    const buttons = itemEl.querySelectorAll('.quantity-btn');
...
function recalcAndUpdateTotals() {
    const cartTotalElement = document.getElementById('cartTotal');
    ...
    cartItems.forEach(item => {
        const subtotalEl = item.querySelector('.cart-item-total');
        if (subtotalEl) {
            const subtotal = parseCurrencyToNumber(subtotalEl.textContent);
            grandTotal += subtotal;
        }
    });
    
    cartTotalElement.textContent = `₹${grandTotal.toLocaleString()}`;
}
